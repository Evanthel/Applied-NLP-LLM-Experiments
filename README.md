...
Hands-on Python experiments focused on NLP, local LLM inference, evaluation, and small deep learning prototypes. The repo is more about practical experimentation than polished apps.
...
- Some scripts still require local datasets or model files that are not committed to the repo.
- `dev_llm/classifying_emails_using_llama.py` expects `LLAMA_MODEL_PATH` to point to a local `.gguf` model file.
...
- `dev_llm/classifying_emails_using_llama.py`
  Expects `data/email_categories_data.csv` and `LLAMA_MODEL_PATH` to reference a local GGUF model file.
...
- `nlp/sentiment_analysis_in_python.py`
  Assumes an in-memory `reviews` DataFrame and `positive_reviews` text prepared before running the script.
- `nlp/nltk.ipynb` and `nlp/spacy.ipynb`
  Expect a local `pg100.txt` copy of the Shakespeare corpus if you want to rerun the analysis.
- `nlp/question_answering_with_web_search.ipynb`
  Requires internet access for Wikipedia lookups.
...
- Notebook-based NLP exercises with NLTK, spaCy, corpus analysis, and lightweight retrieval

## What I learned

- How to turn a natural language problem into a concrete prompting or classification setup
...
- [`nltk.ipynb`](./nlp/nltk.ipynb)
  Combined NLTK notebook covering tokenization, stopword removal, stemming, POS tagging, NER, and Shakespeare corpus analysis.
- [`spacy.ipynb`](./nlp/spacy.ipynb)
  Combined spaCy notebook covering tokenization, lemmatization, POS tagging, sentence similarity, and Shakespeare corpus analysis.
- [`question_answering_with_web_search.ipynb`](./nlp/question_answering_with_web_search.ipynb)
...
- text, audio, and image workflows

That combination makes it a good learning log for practical model intuition, not just a dump of isolated exercises.

## Notes

- Most files are short experimental scripts rather than full production-ready packages.
- Some scripts expect local datasets, downloaded model weights, or local paths that are not included in the repository.
