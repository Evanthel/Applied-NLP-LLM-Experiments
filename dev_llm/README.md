...
- `analyzing_car_reviews_with_llms.py`
  Hugging Face workflow for sentiment analysis, translation, question answering, summarization, and metric-based evaluation.
...
- turning natural-language requests into prompt templates
- comparing pretrained-model workflows with custom neural models
- evaluating outputs with `accuracy`, `f1`, `precision`, and `recall`
- working with local model files and external datasets

## Running notes

- These scripts expect local datasets or model files that are not stored in this repo.
- `classifying_emails_using_llama.py` also expects `LLAMA_MODEL_PATH` to point to a local `.gguf` model file.
